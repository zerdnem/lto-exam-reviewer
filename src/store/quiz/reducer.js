...
function questionsReducer(state = defaultQuestionsState, action) {
  switch (action.type) {
    ...
    case NavActionTypes.startNewExam: {
      const {questions, questionsById, examName} = action.payload;
      const currentQuestionIndex = 0;
      const answers = [];
      return state.merge({
        questions,
...
export const defaultExamListState = Immutable({
  data: [],
});

function examListReducer(state = defaultExamListState, action) {
  switch (action.type) {
    case NavActionTypes.fetchExams: {
